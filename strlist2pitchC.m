function [pitchlist,beatlist,locationlist] = strlist2pitchC(strlist)
%STRLIST2PITCH 音符字符串转换到频率时长音区向量（矩阵），C大调
%   input:音符字符串列表
...
for counts=1:length(strlist)
    ...
    if contains(note,'+')
        ...
        for singleNote=notes
            [notePitch,noteBeat,noteLocation] = pitchbeatC(singleNote);
        ...
        [notePitch,noteBeat,noteLocation] = pitchbeatC(note);
